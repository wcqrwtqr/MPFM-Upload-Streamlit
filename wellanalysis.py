...
def constant_rate_drawdown_test(t, p, q, Bo, mu_oil, h, poro, ct, rw, pi, your_guess=5):
    ...
    def reservoir_size(q, Bo, poro, h, ct, m2):
        ...
        return np.sqrt(-(0.07447 * q * Bo) / (poro * h * ct * m2))
    ...
    plt.ylabel("Pressure (psia)", size=17)
    plt.xlim(0, max(t))
...
def multi_rate_drawdown_test(t, p, t_change, q_change, Bo, mu_oil, h, poro, ct, rw, pi):
    ...
    def skin_factor(k, poro, mu_oil, ct, rw, c, m):
        ...
        """
        return 1.1513 * (
            (c / m) - np.log10(k / (poro * mu_oil * ct * (rw**2))) + 3.2275
        )
    ...
    plt.xlabel(r"Time (hours)", size=17)
    plt.ylabel(r"Pressure (psi)", size=17)
    plt.grid(True, which="both", color="black", linewidth=0.1)
    ...
    plt.xlabel(r"$F_p$", size=17)
    plt.ylabel(r"$\frac{p_i-p_{wf}}{q_n}$ (psi-D/STB)", size=17)
...
def constant_pressure_test(t, q, pwf, pi, Bo, mu_oil, h, poro, ct, rw):
    ...
    plt.xlabel("Time (hour)", size=17)
    plt.ylabel(r"Rate (STB/D)", size=17)
    plt.xlim(0, max(t))
    ...
    plt.xlabel("Time (hour)", size=17)
    plt.ylabel(r"$\frac{1}{q}$ (D/STB)", size=17)
    plt.xlim(xmin=1)
...
def constant_rate_buildup_test(
    t, p, q, Bo, mu_oil, h, poro, ct, rw, t_since_shutin, your_guess=5
):
    """
    ...
    def skin_factor(t_since_shutin, pwf, k, poro, mu_oil, ct, rw, m, pi):
        ...
        return 1.1513 * (
            ((pwf - b) / m1) - np.log10(k / (poro * mu_oil * ct * (rw**2))) + 3.2275
        )
    ...
    plt.plot(t, p, ".", color="black")
    plt.title("Normal Plot of BHFP vs Time", size=20, pad=10)
    plt.xlabel("Time since shut-in (hours)", size=17)
    plt.ylabel("Pressure (psia)", size=17)
    plt.xlim(0, max(t))
    ...
    plt.axvspan(x[-your_guess], 1e25, color="blue", alpha=0.2)
